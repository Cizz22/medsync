...
import { ConnectionSchemaMap, useGetConnectionSchemaMap } from '@/lib/hooks/useGetConnectionSchemaMap';
...
export default function DataSyncConnectionCard({ jobId }: Props): ReactElement {
  ...
  } = useGetJob(account?.neosync_account_id ?? '',account?.access_token ,jobId);
  ...
  } = useGetConnectionSchemaMap(account?.neosync_account_id ?? '',account?.access_token ?? '' ,sourceConnectionId ?? '');
  ...
  const form = useForm({
    ...
    values: getJobSource(data, connectionSchemaDataMap?.schemaMap),
  ...
  useEffect(() => {
    ...
    const js = getJobSource(data, connectionSchemaDataMap?.schemaMap);
    setSelectedTables(
      new Set(
  ...
  const source = connections.find((item:any) => item.id === sourceConnectionId);
...
function getJobSource(
  ...
  const mappings = (job.mappings ?? []).map((mapping:any) => {
