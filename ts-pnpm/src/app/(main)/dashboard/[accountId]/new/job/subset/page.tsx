...
export default function Page({ searchParams }: PageProps): ReactElement {
  ...
  useEffect(() => {
    if (!searchParams?.sessionId) {
      router.push(`/dashboard/${account?.neosync_account_id}/new/job`);
    }
  }, [searchParams?.sessionId]);
  const { toast } = useToast();
  const { data: connectionsData } = useGetConnections(account?.neosync_account_id ?? '', account?.access_token);
  ...
  async function onSubmit(values: SubsetFormValues): Promise<void> {
    ...
    try {
      ...
      if (job.id) {
        router.push(`/dashboard/${account?.neosync_account_id}/jobs/${job.id}`);
      } else {
...
async function createNewJob(
  ...
  // const body2 = new CreateJobRequest({
  //   accountId,
