...
export default function Page({ searchParams }: PageProps): ReactElement {
  ...
  useEffect(() => {
    if (!searchParams?.sessionId) {
      router.push(`/${account?.neosync_account_id}/new/job`);
    }
  ...
  async function onSubmit(_values: ConnectFormValues) {
    router.push(`/dashboard/${account?.neosync_account_id}/new/job/schema?sessionId=${sessionPrefix}`);
  }
...
function splitConnections(connections: ConnectionResponse[]): {
  postgres: ConnectionResponse[];
  mysql: ConnectionResponse[];
  s3: ConnectionResponse[];
} {
  const postgres: ConnectionResponse[] = [];
  const mysql: ConnectionResponse[] = [];
