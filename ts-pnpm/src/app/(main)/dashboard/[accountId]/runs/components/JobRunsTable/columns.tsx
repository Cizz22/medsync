...
import { JobRunResponse } from '@/lib/hooks/useGetJobRuns';
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div>
            <span className="font-medium">
              {formatDateTime(row.getValue<string>('startedAt'))}
            </span>
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const completedAt = row.getValue('completedAt')
          ? formatDateTime(row.getValue<string>('completedAt'))
          : undefined;
        return (
          <div>
